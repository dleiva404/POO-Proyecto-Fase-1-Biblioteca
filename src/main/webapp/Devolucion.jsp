...
<html lang="es">
  <head>
    ...
    <title>Devolución de Materiales - Colegio Don Bosco</title>
          ...
          <tbody>
            <c:forEach var="p" items="${prestamos}">
              <tr>
                ...
                <td>$${p.mora}</td>
                <td>
                  <form action="DevolucionServlet" method="post" class="m-0">
                    <input type="hidden" name="idPrestamo" value="${p.idPrestamo}" />
                    <input type="hidden" name="idMaterial" value="${p.idMaterial}" />
                    <button type="submit" class="btn btn-guardar btn-sm">Devolver</button>
                  </form>
                </td>
              </tr>
            </c:forEach>
